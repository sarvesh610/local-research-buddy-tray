...
class LucidTalkStreamer: NSObject, WhisperProcessorDelegate {
    ...
    private func printUsage() {
        let usage = """
        LucidTalk Real-time Transcription Streamer
        
        Usage: LucidTalkStreamer --start [options]
        ...
        Examples:
            LucidTalkStreamer --start
            LucidTalkStreamer --start --model ./models/ggml-small.bin
            LucidTalkStreamer --start --batch
    ...
    private func stopTranscription() {
        ResponseHandler.log("🛑 stopTranscription() called from LucidTalkStreamer")
        ...
        let signalHandler: @convention(c) (Int32) -> Void = { signal in
            ...
            if signal == SIGINT || signal == SIGTERM {
                ...
                DispatchQueue.main.async {
                    if let streamer = LucidTalkStreamer.shared {
    ...
    static var shared: LucidTalkStreamer?
}
...
struct MindMeetApp {
    static func main() async {
        let streamer = LucidTalkStreamer()
        LucidTalkStreamer.shared = streamer
